...
const UserProfile: React.FC = () => {
  ...
  const passwordDisplay = isPasswordVisible ? user?.password : '*'.repeat(user?.password.length || 0);
  const [startDate, setStartDate] = useState<Date | null>(null);
  const [endDate, setEndDate] = useState<Date | null>(null);
  const [filteredPurchases, setFilteredPurchases] = useState(user?.purchaseHistory || []);
    ...
    <div className="user-profile">
      ...
      <div className='user-profile-content'>
        <div className='user-info'>
          <div className='user-item'><h2>Full Name: {user?.firstName} {user?.lastName}</h2></div>
          <div className='user-item'><p>Email: {user?.email}</p></div>
          <div className='user-item'><p>Username: {user?.username}</p></div>
          <div className='user-item'><p>Date of Birth: {user?.dateOfBirth}</p></div>
